...
public class WeekPlannerRepository(ApplicationDbContext context) : IWeekPlannerRepository
{
    ...
    public async Task<WeekPlanner?> GetWeekPlanner(YearDataId yearDataId, int weekNumber, int termNumber, int year, CancellationToken cancellationToken)
    {
    ...
    public async Task<WeekPlanner?> GetByLessonDate(DateOnly lessonDate, CancellationToken cancellationToken)
    {
        var weekPlanner = await _context.WeekPlanners
            .Where(wp => lessonDate.DayNumber - wp.WeekStart.DayNumber < 5 && lessonDate.DayNumber - wp.WeekStart.DayNumber >= 0)
            .FirstOrDefaultAsync(cancellationToken);

        return weekPlanner;
    }

    public async Task<WeekPlanner?> GetByYearAndWeekNumber(int year, int weekNumber, CancellationToken cancellationToken)
    {
        var weekPlanner = await _context.WeekPlanners
